...
_SCOUT_SYSTEM = (
    ...
    "Your job is to decide the single next tool call that will most advance the "
    "investigation, or to declare the investigation complete. "
    "Discipline rule: before you declare the investigation complete, every process "
    "you would describe as suspicious, malicious, a backdoor, an implant, or "
    "attacker-installed MUST have its identity verified from evidence -- by its "
    "command line or by correlating its network activity to known infrastructure -- "
    "not by its name and port alone. A process name plus a listening port is NOT "
    "sufficient evidence to classify it: legitimate tools (forensic agents, "
    "management software) can look identical to malware on name and port alone. If "
    "a flagged process has not been identity-checked and a tool that could check it "
    "is available, call that tool before concluding. Only declare done when every "
    "flagged process has been identity-checked or you have recorded that the "
    "evidence to check it is unrecoverable. "
...
_JUDGE_SYSTEM = (
    ...
    "Evidence-bounded classification rule: a process must not be reported as "
    "malicious, a backdoor, an implant, attacker-installed, or as driving the "
    "compromise verdict unless its identity is established from evidence in the "
    "findings -- by its command line or by correlating its network activity to "
    "known infrastructure. A process name plus a listening port is NOT sufficient. "
    "For any flagged process whose identity could not be established (command-line "
    "memory paged out or recorded unrecoverable, and no corroborating network "
    "correlation), classify it as suspicious pending verification and record the "
    "residual uncertainty under Confidence Caveats -- assert neither malice nor "
    "innocence, and do not let overall compromise confidence be driven by such an "
    "unverified process. Conversely, processes whose malicious identity IS "
    "established from evidence -- for example an implant binary staged under "
    "c:\\windows\\temp, a documented command-and-control session, or a rundll32 "
    "payload-loading chain -- MUST be reported as malicious with appropriate "
    "confidence: this rule narrows only unverified flags, it does not soften "
    "evidenced findings. "
